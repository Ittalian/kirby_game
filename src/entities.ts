...
export function makePlayer(k: KaboomCtx, posX: number, posY: number) {
    const player = k.make([
        ...
        k.body(), // 重力やコリジョンの影響を受けるようになる
        k.pos(posX * scale, posY * scale),
        k.scale(scale),
        k.doubleJump(10),
        k.health(3),  //HP等を設定できるようになる
        k.opacity(1), //透明度(1で完全に表示される)
        // player.speedのようにすることで、このオブジェクトにアクセスできる
        {
            speed: 300,
            direction: "right",
            isInhaling: false,
...
export function setControls(k: KaboomCtx, player: PlayerGameObj) {
    ...
    k.onKeyPress((key) => {
        // ジャンプに様々なボタンを実装するときのために、switch文にしておく
        switch (key) {
            case "x":
                player.doubleJump();
                break;
...
export function makeFlameEnemy(k: KaboomCtx, posX: number, posY: number) {
    ...
    flame.onStateEnter("jump", async () => {
        // jumpメソッドは、bodyコンポーネントで使用できるようになる(引数はジャンプ力)
        flame.jump(1000);
    });

    flame.onStateUpdate("jump", async () => {
        // isGroundedメソッドは、bodyコンポーネントによって使用できる
        if (flame.isGrounded())
...
export function makeBirdEnemy(k: KaboomCtx, posX: number, posY: number, speed: number) {
    const bird = k.add([
        ...
        k.move(k.LEFT, speed),
        // 見える範囲からの距離が400以上になったらオブジェクトを消去する
